...
Rule definitions for the project-wide Jira Hygiene capability (Manage → Hygiene Rules, Reports → Jira Hygiene). Owned and committed by `org-pulse-data`; `org-pulse` only reads it via `GET /modules/releases/hygiene/project-hygiene/config` — the app never writes this file.
...
Per-rule Jira hygiene violation counts and matching issues, computed by the `org-pulse-data` collector against the config above. Served read-only via `GET /modules/releases/hygiene/project-hygiene`; the app performs no Jira calls or writes for this contract.
