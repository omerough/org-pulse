...
- `effectiveExecutionIssueCount`, `effectiveDoneExecutionIssueCount`, `effectiveExecutionState`,
  ...
  crediting an Epic classified `completedViaStatus` (closed, any resolution) as fully done regardless
  of its children. Consumers should treat `effective*` as the primary source for progress
  bars/lanes/summaries, falling back to the same-named raw field only when the `effective*` key is
  **absent from the object entirely** (a payload predating this contract) — an explicit `null` on a
  present key is real insufficient-data, not a signal to fall back. A `complete`
...
- `metrics.effectiveExecutionIssueCount`, `metrics.effectiveDoneExecutionIssueCount`,
  ...
  `metrics.effectiveExecutionCoverageReason` are the Epic status-completion override of the raw
  fields above — see the Execution Index notes for the full semantics. org-pulse-data computes and
  owns this classification entirely; org-pulse consumes it read-only.
...
**Epic status-completion classification (`completedViaStatus`):** boolean, `true` when the Epic's
`statusCategory` is `Done` (closed, regardless of resolution) — it is then credited as fully done
for `effective*` progress regardless of its actual children's status. Derived by org-pulse-data
purely from `statusCategory`; org-pulse consumes it read-only and must not re-derive it from
`status` itself.
...
older than the one already stored keeps the stored `statusCategory`/`completedViaStatus`/`updated`
rather than reviving a classification a more recent Jira observation already invalidated. Raw issue
data (`issues[]`, execution counts) always comes from the incoming Epic regardless. Epics without a
comparable `updated` on both sides always accept the incoming classification, as before this check
existed.
...
## Releases — Release Plan Index (`data/releases/release-plans/index.json`)

Produced by an `agentic-ci` GitLab CI job (OSAC-4398) running the `/release-plan` skill (OSAC-4397). `org-pulse` is a read-only consumer served via `GET /api/modules/releases/release-plans` — a pure `readFromStorage` passthrough, no LLM call in the app.
...
Structured JSON output of the `/release-plan` skill (schema owned by `osac-ai-skills` PR #19 / `skills/release-plan/references/json-schema.md`) — a forward-looking report of what an OSAC version will deliver, served via `GET /api/modules/releases/release-plan?version=`.

```json
{
  "metadata": { "version": "0.3", "priorVersion": "0.2", "generatedAt": "2026-08-19 14:35", "badge": "Developer Preview" },
  "vision": { "summary": "...", "metrics": [{ "num": 41, "label": "Features in 0.3" }] },
  "serviceMatrix": {
    ...
    "rows": [
      {
        ...
        "cells": {
          "CaaS": [
            { "version": "0.1", "isTarget": false, "text": "Cluster provisioning via HyperShift + Metal3" },
            { "version": "+0.3", "isTarget": true, "text": "Cluster upgrade automation" }
          ],
          "VMaaS": "—",
          "BMaaS": "—",
          "MaaS": "—"
  ...
  "customerCoverage": {
    "ncp": [{ "req": "CNP02", "requirement": "Declarative Resource Interfaces", "coverage": "...", "version": "0.2", "status": "Partial" }],
  ...
  "cumulativeProgression": [
    {
      ...
      "versions": [{ "version": "0.1", "isTarget": false, "items": [{ "jira": "OSAC-1191", "text": "Cluster provisioning via HyperShift + Metal3 + BCM inventory" }] }]
    }
  ],
  "featureInventory": [
    {
      "group": "CaaS — Cluster Provisioning",
      "features": [{ "key": "OSAC-1415", "summary": "Support cluster upgrade", "customers": ["Moc", "Telenor"], "status": "In Review" }]
    },
    {
      "group": "Spikes (Investigations)",
      "features": [{ "key": "OSAC-2850", "summary": "[spike] Investigate Slurm cluster setup using BMaaS API", "customers": ["Moc", "Telefonica"], "status": "Planned" }]
    }
  ],
  "notes": {
...
- `customerCoverage` is an object, not an array: `ncp` (fixed NCP requirements table) plus `byCustomer` (array of `{customer, rows}` for other customers).
- Generated externally by the `agentic-ci` CI job (OSAC-4398); `org-pulse` never computes or caches this data — pure storage passthrough, per the "app is a display layer" hard constraint.
...
## Releases — Project Hygiene Config (`data/releases/hygiene/project-hygiene-config.json`)

Rule definitions for the project-wide Jira Hygiene capability (Manage → Hygiene Rules, Reports → Jira Hygiene). Owned and committed by `org-pulse-data`; `org-pulse` only reads it via `GET /api/modules/releases/hygiene/project-hygiene/config` — the app never writes this file.
      ...
      "rules": [
        {
          ...
          "category": "ownership",
          "jql": "project = OSAC AND cf[10001] is EMPTY AND statusCategory != Done",
          "fields": "summary,status,issuetype,assignee,fixVersions,components,parent,customfield_10001"
        }
      ],
      "fieldMappings": {
...
**Notes:**
- `projects` is keyed by Jira project key (not an array) — supports more than one project without a contract change; only one project is configured today.
- Each project's `rules` array is fully dynamic. The app renders whatever rules are present with no hardcoded rule IDs, categories, or count — this is what makes the Manage and Reports views project-agnostic.
- `fields` is a comma-separated Jira field list passed to the collector's JQL fetch, not a JSON array.
- `fieldMappings` maps a semantic name (e.g. `team`) to the project's actual Jira custom field ID. It's used by the `org-pulse-data` collector to serialize that field into the result contract below — the app does not read `fieldMappings` directly.

## Releases — Project Hygiene Results (`data/releases/hygiene/project-hygiene-results.json`)

Per-rule Jira hygiene violation counts and matching issues, computed by the `org-pulse-data` collector against the config above. Served read-only via `GET /api/modules/releases/hygiene/project-hygiene`; the app performs no Jira calls or writes for this contract.
...
```json
{
  ...
  "results": {
    "OSAC": {
      ...
      "partial": true,
      "errors": [
        { "ruleId": "no-team", "message": "Jira query timed out after 3 retries" }
      ],
      "summary": {
        "uniqueIssueCount": 4,
        "totalRuleMatches": 5,
      ...
      "rules": [
        {
          ...
          "issues": [
            {
              "key": "OSAC-101",
              "summary": "Support disconnected install for VM workloads",
              "issueType": "Epic",
              "status": "In Progress",
              "assignee": "Alex Kim",
              "parentKey": "OSAC-50",
              ...
              "team": null,
              "teamId": null,
              "jiraUrl": "https://redhat.atlassian.net/browse/OSAC-101"
            }
          ]
        },
        {
...
**Notes:**
- `results` is keyed by project key, matching `config.projects` keys — single project today, multi-project-ready without a contract change.
- `configVersion` is the first 12 hex characters of a SHA-256 hash over the raw config file bytes at collection time, used to detect config/result drift.
- `source` is a static label identifying the origin dataset (e.g. the Jira dashboard the rules were modeled on), not derived per-request.
- `summary.uniqueIssueCount` is the count of distinct issue keys across all successful rules — an issue matching two rules counts once. `summary.totalRuleMatches` is the sum of successful rules' `count` — an issue matching two rules adds to both. The two values diverge whenever any issue matches more than one rule; they are equal only when no issue matches more than one rule.
- `summary.affectedRuleCount` counts rules with `count > 0`; `summary.failedRuleCount` counts rules with `count: -1`.
- `partial: true` means at least one rule's JQL failed to execute. `errors[]` then holds one `{ "ruleId", "message" }` entry per failed rule, and that rule's entry in `rules[]` has `count: -1` and `issues: []` — this is a distinct state from a verified zero-match rule (`count: 0`, `issues: []`), and the app must not conflate "collection failed" with "no violations found." Rules that succeeded are unaffected by another rule's failure.
- Each `rules[]` entry copies `name`/`description`/`category` from the config at collection time, so the report renders without a second fetch.
- Each issue's `team` and `teamId` may both be `null` when the Jira Team custom field is empty on that issue. `teamId` is populated only when the field returns an object with a stable `id`; a plain string value (or an empty field) yields `teamId: null` with `team` set from the string.
- A single Jira issue can appear in more than one rule's `issues[]` array if it violates multiple rules — this contract does not deduplicate. The app is responsible for merging issues by `key` when rendering a unique-issue view (e.g. Reports → Jira Hygiene's Issues tab).
- `jiraUrl` is precomputed by the collector as `{jiraBaseUrl}/browse/{key}` — the app must use it as-is, not reconstruct it.
