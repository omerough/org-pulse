...
const metrics = computed(() => {
  ...
  const hasUnknown = windowIssues.some(i => i.wasEligible === null || i.wasEligible === undefined)
  const eligibleCount = hasUnknown ? null
    : windowIssues.filter(i => i.wasEligible === true).length
  const eligibilityRate = hasUnknown ? null
    : windowIssues.length > 0 ? Math.round((eligibleCount / windowIssues.length) * 100)
    : 0
        ...
        <div class="p-6 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <div class="relative bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 text-center">
            <div class="absolute top-2 right-2 group">
              ...
              <div class="absolute right-0 top-6 z-20 hidden group-hover:block w-56 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg dark:shadow-gray-900/50 p-3 text-xs text-gray-700 dark:text-gray-300 text-left">
                Percentage of all issues ever determined eligible for autofix (received the <span class="font-mono">jira-autofix</span> label). Calculated as: <span class="font-medium">eligible ÷ total × 100</span>.
              </div>
            </div>
            <div class="text-2xl font-bold text-indigo-600 dark:text-indigo-400">
              {{ metrics.eligibilityRate !== null ? metrics.eligibilityRate + '%' : '--' }}
            </div>
            ...
            <div class="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5">
              <template v-if="metrics.eligibleCount !== null">{{ metrics.eligibleCount }} eligible of {{ metrics.windowTotal }} total</template>
